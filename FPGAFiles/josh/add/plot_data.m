...
fig_num = 1;
fig_num = make_plots(Adders_32, '16 Bit Equivalent', fig_num, {'r4','r8','control'});
fig_num = make_plots(Adders_64, '32 Bit Equivalent', fig_num, {'r4','r8','control'});
fig_num = make_plots(Adders_128, '64 Bit Equivalent', fig_num, {'r4','r8','control'});
fig_num = make_plots(Adders_256, '128 Bit Equivalent', fig_num, {'r4','r8','control'});
...
function fig_num = make_plots(Adders, width, fig_num, data_lgd)
    ...
    nexttile(1);
    title(sprintf('Average Error %s',width))
    xlabel('Freqeuncy (MHz)')
    ylabel('Error')
    nexttile(2);
    title(sprintf('Maximum Error %s',width))
    xlabel('Freqeuncy (MHz)')
    ylabel('Error')
    nexttile(3);
    title(sprintf('Minimum Error %s',width))
    xlabel('Freqeuncy (MHz)')
    ylabel('Error')
    hold off
    lgd = legend(data_lgd);
    ...
    nexttile(1);
    title(sprintf('Average Absolute Error %s',width))
    xlabel('Freqeuncy (MHz)')
    ylabel('Absolute Error')
    nexttile(2);
    title(sprintf('Maximum Absolute Error %s',width))
    xlabel('Freqeuncy (MHz)')
    ylabel('Absolute Error')
    hold off
    lgd = legend(data_lgd);
    ...
    ylim([0 15])
%     nexttile(2);
%     title(sprintf('Maximum Mean Relative Error %s',width))
%     xlabel('Freqeuncy (MHz)')
%     ylabel('Mean Relative Error')
%     hold off
    lgd = legend(data_lgd);
