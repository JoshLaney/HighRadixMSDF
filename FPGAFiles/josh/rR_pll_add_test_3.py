...
print 'found f_max of', f_min, 'in', timing, 'seconds'
...
err_file.write('freq, min, max, avg\n')
abs_err_file.write('freq, max, avg\n')
mr_err_file.write('freq, max, avg\n')
bf_loc_file.write('freq, min, min_avg, max, illegal\n')
...
while(ovc_freq<=600):
    ...
    err_file.write('%d, %d, %d, %f\n' %(ovc_freq,min_err,max_err,avg_err))
    abs_err_file.write('%d, %d, %f\n' %(ovc_freq,max_abs_err,avg_abs_err))
    mr_err_file.write('%d, %f, %f\n' %(ovc_freq,max_mr_err,avg_mr_err))
    bf_loc_file.write('%d, %d, %f, %d, %d\n' %(ovc_freq,min_bf,avg_min_bf,max_bf,bf_illegal))
    ovc_freq = ovc_freq+1
...
end_time = time.time()
timing = int(end_time-start_time)
print 'Testing completed in',timing,'seconds... Goodbye!'
