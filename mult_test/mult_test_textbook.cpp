...
int main(){
	...
	for(int test = 1; test<=TESTS; test++){
		...
		for(int j = -3; j<N; j++){
			...
			online_add(x_j[j+3],y_j[(j+1)+3],x_add_y, N+M, true);
			shift_up2(w, N+M);
			...
			myfile<<"	j="<<j<<"\n";
			myfile<<"	x[j]y  :";
			print_vec(x_j[j+3], N+M,N+M-2);
			myfile<<"	y[j+1]x:";
			print_vec(y_j[(j+1)+3], N+M,N+M-2);
			myfile<<"	x+y    :";
			print_vec(x_add_y, N+M,N+M-2);
			myfile<<"	v      :";
			print_vec(v, N+M, N+M-2);
			if(j>=0){
				myfile<<"	v_est  :";
				print_vec(v_est, 4, 2);
				myfile<<"	(p_j ="<<sel<<")\n";
		...
		#endif
		online_add(p, p_frac, p, 2*N, true);
